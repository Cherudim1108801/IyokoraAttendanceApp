...
public class SongParticipation
{
    ...
    /// <summary>この練習におけるこの曲の録音音源（複数・名前付きで登録可能）。</summary>
    public List<PracticeRecording> Recordings { get; init; } = [];

    /// <summary>録音が1件以上登録済みかどうか。</summary>
...
public class ParticipationDot
{
    ...
    /// <summary>パートの区切りを示すための余白（パートが切り替わる先頭の○のみ広めに取る）。</summary>
    public Thickness Margin { get; init; } = new(4);
}
